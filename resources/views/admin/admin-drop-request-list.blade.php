        ...
        <main class="main-content">
            ...
            <div style="margin-top: 20px;">
                ...
                <div class="summary-card admin-summary-card"
                    style="max-width: 260px; background: linear-gradient(90deg, #dc3545, #ef4444); margin-bottom: 24px;">
                            ...
                            @if(isset($dropRequests) && $dropRequests->count() > 0)
                                @foreach($dropRequests as $dropRequest)
                                    ...
                                    <tr style="border-bottom: 1px solid #e9ecef;">
                                        ...
                                        <td style="padding: 12px 16px; color: #212529; font-size: 14px;">
                                            {{ $dropRequest->subject_code }}</td>
                                        <td style="padding: 12px 16px; color: #212529; font-size: 14px;">
                                            {{ $subject ? $subject->title : '-' }}</td>
                                        <td style="padding: 12px 16px; color: #212529; font-size: 14px;">
                                            {{ $subject ? number_format($subject->units, 1) : '-' }}</td>
                                        <td style="padding: 12px 16px; color: #212529; font-size: 14px;">
                                            {{ $dropRequest->reason }}</td>
                                        <td style="padding: 12px 16px; color: #212529; font-size: 14px;">
                                            {{ $dropRequest->created_at->format('m/d/y') }}</td>
                                        <td style="padding: 12px 16px;">
                                            @if($dropRequest->status === 'Pending')
                                                <form method="POST"
                                                    action="{{ route('admin.drop-request.reject', $dropRequest->id) }}"
                                                    style="display: inline; margin-right: 8px;">
                                                    @csrf
                                                    <button type="submit" class="reject-btn"
                                                        onclick="return confirm('Are you sure you want to reject this drop request?')"
                                                        style="background-color: #dc3545; color: white; border: none; padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 500; cursor: pointer; font-family: 'Inter', sans-serif;">Reject</button>
                                                </form>
                                                <form method="POST"
                                                    action="{{ route('admin.drop-request.approve', $dropRequest->id) }}"
                                                    style="display: inline;">
                                                    @csrf
                                                    <button type="submit" class="approve-btn"
                                                        onclick="return confirm('Are you sure you want to approve this drop request?')"
                                                        style="background-color: #28a745; color: white; border: none; padding: 6px 12px; border-radius: 4px; font-size: 12px; font-weight: 500; cursor: pointer; font-family: 'Inter', sans-serif;">Approve</button>
                                                </form>
                                            @else
